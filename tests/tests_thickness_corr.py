...
from src.thickness_correction import *
...
# Patch SliceThicknessCorrection.Pair for testing
import tests.tests_thickness_corr as ttc
ttc.SliceThicknessCorrection = type('SliceThicknessCorrection', (), {'Pair': DummyPair})
...
class TestThicknessCorr(unittest.TestCase):
    ...
    def test_sort_by_monotonically_increasing_z(self):
        ...
        sorted_pairs = sort_by_monotonically_increasing_z(pairs)
    ...
    def test_resample_info_attributes(self):
        ri = ResampleInfo(np.float64(1.5), np.float64(2.5), Path("file.tif"))
